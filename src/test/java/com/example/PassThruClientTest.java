...
/**
 * Unit test for PassThruClient
 */
public class PassThruClientTest extends TestCase {
  private static final int Port1 = 5001;
  private static final int Port2 = 5002;
  private ExecutorService service;
  private CountDownLatch shutdownTriggered1 = new CountDownLatch(1);
  private CountDownLatch shutdownTriggered2 = new CountDownLatch(1);
  ...
  private CompletionHandler acceptHandler1 =
    new CompletionHandler<AsynchronousSocketChannel, PassThruServerSocket>() {
      public void completed(AsynchronousSocketChannel ch, PassThruServerSocket attachment) {
        try {
          new PassThruClient(AsynchronousChannelGroup.withThreadPool(service), ch, "localhost", 5002);
      ...
      public void failed(Throwable ex, PassThruServerSocket attachment) {
        shutdownTriggered1.countDown();
      }
    };
  private CompletionHandler acceptHandler2 =
    new CompletionHandler<AsynchronousSocketChannel, PassThruServerSocket>() {
      public void completed(AsynchronousSocketChannel ch, PassThruServerSocket attachment) {
  ...
  public PassThruClientTest(String testName) {
    super(testName);
  }

  public static Test suite() {
    return new TestSuite(PassThruClientTest.class);
  }
  ...
  public void testPassThruClient() {
    PassThruServerSocket serverSocket1 = new PassThruServerSocket(acceptHandler1);
    PassThruServerSocket serverSocket2 = new PassThruServerSocket(acceptHandler2);
    try {
      serverSocket1.start(AsynchronousChannelGroup.withThreadPool(service), Port1);
      serverSocket1.accept();
      serverSocket2.start(AsynchronousChannelGroup.withThreadPool(service), Port2);
      serverSocket2.accept();
      PassThruSocket socket = new PassThruSocket(AsynchronousChannelGroup.withThreadPool(service),
          connectHandler,
          readHandler,
          writeHandler);
      socket.connect(new InetSocketAddress("localhost", Port1));
      ...
      exitTriggered.await();
      serverSocket2.stop();
      shutdownTriggered2.await();
      serverSocket1.stop();
      shutdownTriggered1.await();
      socket.stop();
      PassThruClient.shutdown();
      assertEquals("012345678910111213141516exit", received.toString());
    } catch (IOException ex) {
      System.out.println("IOException: " + ex.getMessage());
